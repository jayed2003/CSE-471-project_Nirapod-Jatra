...
function fallbackRoute(start: Point, end: Point) {
  ...
  const formula =
    Math.sin(lat / 2) ** 2 +
    Math.cos(radians(start[1])) * Math.cos(radians(end[1])) * Math.sin(lon / 2) ** 2;
  ...
  return {
    ...
    durationMin: Math.max(1, Math.round(meters / 11 / 60)),
    instructions: [],
    isFallback: true,
    score: 100,
    recommended: true,
...
function scoreRoute(distanceKm: number, durationMin: number, aqi: number, weatherAlert: boolean) {
  return Math.round(
    distanceKm * 1.5 +
      durationMin * 0.7 +
      (aqi > 150 ? 35 : aqi > 100 ? 18 : aqi > 50 ? 7 : 0) +
      (weatherAlert ? 20 : 0),
  );
}
...
export async function getRoutePlan(
...
): Promise<{ routes: NormalizedRoute[] }> {
  ...
  try {
    ...
    const sourceRoutes = (
      (await upstream.json()) as {
        routes?: Array<{
          ...
          duration: number;
          legs: Array<{
            steps: Array<{ name: string; maneuver: { type: string; modifier?: string } }>;
          }>;
        }>;
      }
    ).routes;
    if (!sourceRoutes?.length) return { routes: [fallbackRoute(start, end)] };
    const routes: NormalizedRoute[] = sourceRoutes.slice(0, 3).map((route) => {
      ...
      return {
        ...
        isFallback: false,
        score: scoreRoute(distanceKm, durationMin, aqi, weatherAlert),
        recommended: false,
      };
    });
    ...
    return { routes: [fallbackRoute(start, end)] };
  }
}
