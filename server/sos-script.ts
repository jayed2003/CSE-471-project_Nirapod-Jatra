...
> = {
  medical: {
    ...
    service: "ambulance",
    serviceBn: "অ্যাম্বুলেন্স",
    followUpBn: ["রোগীর বয়স কত?", "রোগী কি জ্ঞান হারিয়েছে?", "শ্বাস নিতে পারছে কি?"],
    followUpEn: ["How old is the patient?", "Is the patient conscious?", "Are they breathing?"],
  },
  accident: {
    bn: "সড়ক দুর্ঘটনা",
    en: "road accident",
    service: "ambulance",
    serviceBn: "অ্যাম্বুলেন্স",
    followUpBn: ["কতজন আহত?", "কেউ কি গাড়িতে আটকে আছে?", "রাস্তা কি বন্ধ হয়ে গেছে?"],
    followUpEn: [
  ...
  fire: {
    ...
    en: "fire",
    service: "fire",
    serviceBn: "ফায়ার সার্ভিস",
    followUpBn: ["আগুন কোন তলায়?", "ভেতরে কেউ আটকে আছে?", "গ্যাস সিলিন্ডার আছে কি?"],
    followUpEn: [
      "Which floor is the fire on?",
      "Is anyone trapped inside?",
      "Are there gas cylinders nearby?",
    ],
  },
  flood: {
    bn: "বন্যার পানিতে আটকে আছি",
    en: "trapped by floodwater",
    service: "fire",
    serviceBn: "ফায়ার সার্ভিস",
    followUpBn: ["পানির উচ্চতা কত?", "সাথে কতজন আছেন?", "শিশু বা বয়স্ক কেউ আছে?"],
    followUpEn: [
  ...
  crime: {
    bn: "আমি আক্রমণের শিকার হয়েছি",
    en: "assault or crime in progress",
    service: "police",
    serviceBn: "পুলিশ",
    followUpBn: ["আক্রমণকারী কি এখনো সেখানে আছে?", "কেউ কি আহত?", "অস্ত্র আছে কি?"],
    followUpEn: ["Is the attacker still there?", "Is anyone injured?", "Are there weapons?"],
  },
  harassment: {
    bn: "আমি হয়রানির শিকার হচ্ছি এবং নিরাপদ বোধ করছি না",
    en: "harassment, I do not feel safe",
    service: "police",
    serviceBn: "পুলিশ",
    followUpBn: ["আপনি কি এখন নিরাপদ জায়গায় আছেন?", "অভিযুক্তকে চেনেন?", "আশেপাশে লোকজন আছে?"],
    followUpEn: [
  ...
  stranded: {
    bn: "আমি আটকে পড়েছি, নিরাপদ জায়গায় যেতে পারছি না",
    en: "stranded and unable to reach safety",
    service: "any",
    serviceBn: "জরুরি সেবা",
    followUpBn: ["সাথে কতজন আছেন?", "খাবার ও পানি আছে কি?", "ফোনের চার্জ কতটুকু?"],
    followUpEn: [
      "How many people are with you?",
      "Do you have food and water?",
      "How much phone battery is left?",
    ],
  },
  unknown: {
    bn: "জরুরি অবস্থা",
    ...
    service: "any",
    serviceBn: "জরুরি সেবা",
    followUpBn: ["ঠিক কী ঘটেছে?", "কেউ কি আহত?", "আপনি কি নিরাপদ?"],
    followUpEn: ["What exactly happened?", "Is anyone injured?", "Are you safe?"],
  },
};
  ...
  return value
    ...
    .map((character) =>
      character === "."
        ...
        : character === "-"
          ? "ঋণাত্মক"
          : BN_DIGIT_WORDS[Number(character)],
    )
    .join(" ");
}
...
export function speakCoordinateEn(value: number) {
  return value
    .toFixed(4)
    .split("")
    .map((character) => (character === "." ? "point" : character === "-" ? "minus" : character))
    .join(" ");
}
...
export function bearingBetween(from: [number, number], to: [number, number]) {
  ...
  const x =
    ...
    Math.sin(toRadians(from[1])) * Math.cos(toRadians(to[1])) * Math.cos(deltaLongitude);
...
function landmarkCategory(elementTags: Record<string, string>) {
  return (
    elementTags.amenity ??
    elementTags.shop ??
    elementTags.man_made ??
    (elementTags.highway === "bus_stop" ? "bus_stop" : undefined) ??
    "place"
  );
}
...
export function isRecognisableName(name: string) {
  return name
    .split(/[\s,'-]+/)
    .some((token) => token.length > 2 && !GENERIC_NAME_TOKENS.has(token.toLowerCase()));
}
      ...
      try {
        ...
        for (const element of body.elements) {
          ...
          scored.push({
            ...
            score: ((LANDMARK_WEIGHT[category] ?? 0.5) * recognisability) / (1 + distanceKm),
          });
        }
        if (scored.length)
          return scored
...
export async function reverseGeocode(
  latitude: number,
  longitude: number,
): Promise<ScriptAddress | null> {
  const cacheKey = `${latitude.toFixed(4)},${longitude.toFixed(4)}`;
  const cached = reverseGeocodeCache.get(cacheKey);
  if (cached && Date.now() - cached.savedAt < REVERSE_GEOCODE_CACHE_MS) return cached.address;
  try {
    const upstream = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=jsonv2&zoom=17&lat=${latitude}&lon=${longitude}`,
      {
...
export function shortAddress(address: ScriptAddress | null) {
  ...
  const parts = [address.road, address.area, address.city].filter((part): part is string =>
    Boolean(part),
  );
  return parts.length
    ? [...new Set(parts)].join(", ")
    : address.displayName.split(",").slice(0, 3).join(",").trim();
}
...
export function composeSosScript(
  facts: SosScriptFacts,
  ...
  const banglaLines = [
    ...
    landmark
      ? `কাছের পরিচিত জায়গা ${landmark.name} — সেখান থেকে প্রায় ${distanceBn} মিটার ${landmark.bearingBn}।`
      : "কাছাকাছি কোনো পরিচিত চিহ্ন নিশ্চিত করা যায়নি।",
  ...
  const englishLines = [
    ...
    landmark
      ? `The nearest landmark is ${landmark.name} — about ${landmark.distanceM} metres ${landmark.bearingEn} of it.`
      : "No nearby landmark could be confirmed.",
  ...
  const landmarkSms = landmark
    ? `${landmark.name} theke ~${landmark.distanceM}m ${landmark.bearingEn}. `
    : "";
  const smsLatin = clamp(
    `SOS: ${situation.en}. ${callerName}. ${landmarkSms}${place ? `${place}. ` : ""}${facts.mapsUrl}`,
    SMS_LATIN_LIMIT,
  );
  const smsBangla = clamp(
    `জরুরি: ${situation.bn}। ${callerName}। ${landmark ? `${landmark.name} থেকে ${distanceBn} মি ${landmark.bearingBn}। ` : ""}${facts.mapsUrl}`,
    SMS_BANGLA_LIMIT,
  );
  ...
  const plain = [
    ...
    landmark
      ? `Nearest landmark: ${landmark.name}, about ${landmark.distanceM} m ${landmark.bearingEn} of it`
      : "Nearest landmark: not confirmed",
    facts.alternatives.length
      ? `Other landmarks: ${facts.alternatives.map((alternative) => `${alternative.name} (${alternative.distanceM} m)`).join("; ")}`
      : null,
    facts.nearestHospital
      ? `Nearest hospital: ${facts.nearestHospital.name} (${facts.nearestHospital.distanceKm} km)`
      : null,
    note ? `Note: ${note}` : null,
    `Map: ${facts.mapsUrl}`,
    degraded
      ? "NOTE: landmark/address lookup was unavailable — coordinates are still exact."
      : null,
  ]
    .filter((line): line is string => Boolean(line))
    .join("\n");
...
export async function buildSosScript(
  input: z.infer<typeof sosScriptSchema> & { callerName: string },
): Promise<SosScript> {
  ...
  const facts: SosScriptFacts = {
    ...
    situation: {
      type: input.situationType,
      bn: situationEntry.bn,
      en: situationEntry.en,
      service: situationEntry.service,
      serviceBn: situationEntry.serviceBn,
    },
