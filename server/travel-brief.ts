...
function fallbackBrief(context: BriefContext) {
  ...
  const journey = context.durationMin
    ? `Allow about ${context.durationMin} minutes for the journey to ${destination}.`
    : `Confirm your departure time before travelling to ${destination}.`;
  const conditions = context.weather
    ? `Pack for ${context.weather.toLowerCase()} conditions${context.temperature !== undefined ? ` near ${Math.round(context.temperature)} C` : ""}.`
    : "Check local conditions again before departure.";
  const air = context.aqiLabel
    ? `Air quality is currently listed as ${context.aqiLabel}; carry water and take comfort breaks as needed.`
    : "Keep your phone charged and share your route with a trusted contact.";
  return `- ${journey}\n- ${conditions}\n- ${air}`;
}

export async function buildTravelBrief(
  context: BriefContext,
): Promise<{ brief: string; source: "gemini" | "fallback" }> {
  ...
  try {
    ...
    return { brief: fallbackBrief(context), source: "fallback" };
  }
}
...
export async function buildBestTimeToVisit(
  destination: string,
  latitude: number,
  ...
  try {
    ...
    return { text: fallbackBestTimeToVisit(destination, latitude), source: "fallback" };
  }
}
