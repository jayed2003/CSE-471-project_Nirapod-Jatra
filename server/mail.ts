...
async function getTransporter(): Promise<Transporter | null> {
  ...
  if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
    ...
    transporter = Promise.resolve(nodemailer.createTransport({ host: SMTP_HOST, port, secure: port === 465, auth: { user: SMTP_USER, pass: SMTP_PASS } }));
    return transporter;
  }
  ...
  transporter = nodemailer.createTestAccount().then((account) => {
    usingEthereal = true;
    console.warn("SMTP not configured: routing emails through a temporary Ethereal test inbox (not delivered to real recipients). Set SMTP_HOST/SMTP_USER/SMTP_PASS to send real email.");
...
async function sendPlainTextEmail(to: string, subject: string, lines: Array<string | null>, replyTo: string | undefined, logLabel: string): Promise<SosEmailResult> {
  const client = await getTransporter();
  if (!client) {
    console.warn(`${logLabel} not sent to ${to}: SMTP is not configured (set SMTP_HOST/SMTP_USER/SMTP_PASS)`);
    return { sent: false };
  }
  try {
...
export async function sendLocationShareEmail(to: string, params: LocationShareEmailParams): Promise<SosEmailResult> {
  const { requesterName, requesterEmail, shareUrl, expiresAt } = params;
  return sendPlainTextEmail(to, `${requesterName} is sharing their live location with you`, [
    ...
    requesterEmail ? `\nReply to this email to reach ${requesterName} directly at ${requesterEmail}.` : null,
  ], requesterEmail, "Location share email");
}
