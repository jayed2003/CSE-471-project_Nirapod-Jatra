...
app.post("/api/auth/register", rateLimit({ windowMs: 60_000, limit: 5 }), async (request, response, next) => { try { const input = z.object({ email: z.string().email(), displayName: z.string().min(2).max(80), password: z.string().min(12).max(128), deviceName: z.string().min(2).max(80).default("Web browser"), locationMonitoringEnabled: z.boolean().default(false) }).parse(request.body); const passwordHash = await bcrypt.hash(input.password, 12); const user = await User.create({ email: input.email, displayName: input.displayName, passwordHash, locationMonitoringEnabled: input.locationMonitoringEnabled, trustedDevices: [{ name: input.deviceName }] }); response.status(201).json({ token: issueToken(String(user._id)), user: { id: user._id, email: user.email, displayName: user.displayName } }); } catch (error) { if (error instanceof Error && "code" in error && error.code === 11000) return response.status(409).json({ error: "An account with that email already exists" }); next(error); } });
...
app.post("/api/trips", requireAuth, async (request: AuthRequest, response, next) => { try { const input = tripSchema.parse(request.body); const risk = await fetchLiveRisk(input.destinationPoint); const route = await osrmRoute(input.origin ?? input.destinationPoint, input.destinationPoint); const trip = await Trip.create({ userId: request.userId, destination: input.destination, destinationPoint: input.destinationPoint, travelDates: input.travelDates, route, currentRiskBrief: buildBrief(risk), riskHistory: [{ ...risk, summary: buildBrief(risk) }], shadowProfile: { lastLocation: { type: "Point", coordinates: input.origin ?? input.destinationPoint }, lastUpdated: new Date(), remainingRoute: route.geometry } }); response.status(201).json(trip); } catch (error) { next(error); } });
