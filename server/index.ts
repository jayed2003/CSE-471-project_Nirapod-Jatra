...
app.get("/api/safe-word/suggestions", (_request, response) =>
  response.json({ suggestions: SUGGESTED_SAFE_WORDS }),
);
app.put(
  "/api/me/safe-word",
  requireAuth,
...
app.get("/api/sos/situations", (_request, response) =>
  response.json({
    situations: SITUATION_TYPES.map((type) => ({
      type,
      bn: SITUATIONS[type].bn,
      en: SITUATIONS[type].en,
      service: SITUATIONS[type].service,
      serviceBn: SITUATIONS[type].serviceBn,
    })),
  }),
);
      ...
      const results = await Promise.all(
        contacts.map((contact) =>
          sendSosAlertEmail(contact.email, {
            ...
            script: script?.plain,
            situation: SITUATIONS[input.situationType as SituationType].en,
          }),
        ),
      );
