...
export function describeSafeWordProblem(phrase: string): string | null {
  ...
  if (normalized.replace(/\s+/g, "").length < 5)
    return "Safe-word is too short — it will trigger by accident. Use at least five letters.";
  if (COMMON_PHRASES.has(normalized))
    return "That phrase is too common in ordinary speech. Pick something you would never say by chance.";
  if (
    normalized.split(" ").some((token) => COMMON_PHRASES.has(token)) &&
