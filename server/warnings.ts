...
const SEVERITY_LEVEL: Record<string, "None" | "Watch" | "Warning"> = {
  Minor: "Watch",
  Moderate: "Watch",
  Severe: "Warning",
  Extreme: "Warning",
...
function firstTag(xml: string, tag: string) {
  return tags(xml, tag)[0]?.trim() ?? "";
}

function parsePolygons(xml: string) {
  return tags(xml, "cap:polygon").map((polygon) =>
    polygon
      .trim()
      .split(/\s+/)
      .map((pair) => {
    ...
    await Promise.all(
      items.slice(0, 12).map(async (item) => {
        ...
        try {
          ...
          alerts.push({
            provider: "bmd",
            id: firstTag(xml, "cap:identifier") || item.link,
            event: firstTag(xml, "cap:event"),
            severity: firstTag(xml, "cap:severity"),
            headline: firstTag(xml, "cap:headline") || item.title,
            instruction: firstTag(xml, "cap:instruction"),
            area: firstTag(xml, "cap:areaDesc"),
            effective: firstTag(xml, "cap:effective"),
...
function curatedFloodWarnings(): FloodWarning[] {
  const demo = (process.env.DEMO_FLOOD_STATIONS ?? "")
    .split(",")
  ...
  return FLOOD_STATIONS.map((station) => {
    ...
    return {
      ...
      status,
      headline: demoActive
        ? `${station.station} river gauge is above the danger level (${levelM.toFixed(2)} m).`
        : "River level is below the danger level.",
      source: demoActive ? "demo" : "fallback",
    };
  });
}
...
async function ffwcFloodWarnings(): Promise<FloodWarning[] | null> {
  try {
    ...
    const response = await fetch(
      `https://api.ffwc.gov.bd/data_load/observed-waterlevel-by-station-and-date/10/${today}`,
    ...
    return records.map((record) => {
      ...
      return {
        provider: "bwdb",
        stationId: String(record.station_id ?? record.stationId ?? ""),
        station: String(record.station_name ?? record.station ?? "Gauge"),
        district: String(record.district ?? ""),
        point: [longitude, latitude] as [number, number],
        levelM,
        dangerLevelM,
        status: levelM > dangerLevelM ? "Warning" : levelM > dangerLevelM - 1 ? "Watch" : "None",
        headline: `${String(record.station_name ?? record.station ?? "Gauge")} river gauge reading`,
        source: "ffwc" as const,
      };
    });
  } catch {
    return null;
...
export function warningsNearRoute(
  geometry: unknown,
  alerts: BmdAlert[],
  floods: FloodWarning[],
  corridorKm = FLOOD_CORRIDOR_KM,
): RouteWarning[] {
  ...
  for (const alert of alerts) {
    for (const polygon of alert.polygons) {
      ...
      if (
        polygonBounds.maxLng < bounds.minLng ||
        polygonBounds.minLng > bounds.maxLng ||
        polygonBounds.maxLat < bounds.minLat ||
        polygonBounds.minLat > bounds.maxLat
      )
        continue;
      ...
      if (matched) {
        warnings.push({
          ...
          status: SEVERITY_LEVEL[alert.severity] ?? "Watch",
  ...
  for (const flood of floods) {
    ...
    if (nearest && nearest.distanceKm <= corridorKm)
      warnings.push({
        provider: "bwdb",
        event: "Flood",
        severity: flood.status === "Warning" ? "Severe" : "Moderate",
        status: flood.status,
        headline: flood.headline,
        area: `${flood.station}, ${flood.district}`,
        distanceKm: Math.round(nearest.distanceKm * 10) / 10,
        matchedAt: flood.point,
      });
  }
  return warnings;
}
...
function nearestCuratedShelter(points: Array<[number, number]>): NearestShelter | null {
  ...
  for (const shelter of CURATED_SHELTERS) {
    ...
    if (!nearest || best < nearest.distanceKm)
      nearest = {
        name: shelter.name,
        point: shelter.point,
      ...
      try {
        const upstream = await fetch(instance, {
          ...
          headers: {
            ...
            "user-agent": "WaymarkSafety/1.0 (local development)",
          },
          body: query,
        ...
        for (const element of body.elements) {
          ...
          let best = Infinity;
          for (const point of points)
            best = Math.min(best, haversineKm(point, [longitude, latitude]));
          if (!nearest || best < nearest.distanceKm)
            nearest = {
              name: element.tags?.name ?? "Nearest shelter",
...
function nearestCuratedHospital(
  point: [number, number],
): { name: string; point: [number, number]; distanceKm: number } | null {
...
function looksLikeFacility(name: string) {
  return /\b(hospital|medical|clinic|health|care|nursing|diagnostic|sadar|shishu|chamber|infirmary|general|college)\b/i.test(
    name,
  );
}

export async function nearestHospital(
  point: [number, number],
): Promise<{ name: string; point: [number, number]; distanceKm: number } | null> {
  ...
  const attempt = (async () => {
    for (const instance of OVERPASS_INSTANCES) {
      try {
        ...
        let nearest: { name: string; point: [number, number]; distanceKm: number } | null = null;
        let nearestFacility: { name: string; point: [number, number]; distanceKm: number } | null =
          null;
        for (const element of body.elements) {
          ...
          const candidate = {
            name,
            point: [longitudeValue, latitudeValue] as [number, number],
            distanceKm: Math.round(distanceKm * 10) / 10,
          };
          if (!nearest || distanceKm < nearest.distanceKm) nearest = candidate;
          if (
            looksLikeFacility(name) &&
            (!nearestFacility || distanceKm < nearestFacility.distanceKm)
          )
...
function categorizeEmergencyTags(
  elementTags: Record<string, string>,
): EmergencyServiceCategory | null {
  if (
    elementTags.amenity === "hospital" ||
    elementTags.amenity === "clinic" ||
    elementTags.healthcare === "hospital" ||
    elementTags.healthcare === "clinic"
  )
    return "hospital";
...
function extractPhones(elementTags: Record<string, string>): string[] {
  const raw =
    ...
    elementTags["emergency:phone"] ??
    elementTags.mobile ??
    elementTags["contact:mobile"];
  if (!raw) return [];
  return raw
    .split(/[;,]/)
...
const EMERGENCY_SERVICES_CACHE = new Map<
  string,
  { savedAt: number; services: NearbyEmergencyService[] }
>();
const EMERGENCY_SERVICES_CACHE_MS = 3 * 60 * 1000;

export type NearbyEmergencyServicesResult = {
  services: NearbyEmergencyService[];
  degraded: boolean;
};

export async function nearbyEmergencyServices(
  point: [number, number],
  radiusMeters = 400,
): Promise<NearbyEmergencyServicesResult> {
  const [longitude, latitude] = point;
  ...
  if (cached && Date.now() - cached.savedAt < EMERGENCY_SERVICES_CACHE_MS)
    return { services: cached.services, degraded: false };
  ...
  const queryInstance = async (instance: string): Promise<NearbyEmergencyService[] | null> => {
    try {
      const upstream = await fetch(instance, {
        ...
        body: query,
        signal: AbortSignal.timeout(perInstanceTimeoutMs),
      });
      ...
      for (const element of body.elements) {
        ...
        const distanceMeters = Math.round(
          haversineKm(point, [elementLongitude, elementLatitude]) * 1000,
        );
        if (distanceMeters > radiusMeters) continue;
        services.push({
          id: `${element.type}/${element.id}`,
          name: elementTags.name ?? category[0].toUpperCase() + category.slice(1),
          category,
          point: [elementLongitude, elementLatitude],
          distanceMeters,
          phones: extractPhones(elementTags),
        });
      }
      const deduped: NearbyEmergencyService[] = [];
      for (const candidate of services.sort(
        (a, b) => b.phones.length - a.phones.length || a.distanceMeters - b.distanceMeters,
      )) {
        const isDuplicate = deduped.some(
          (existing) =>
            existing.category === candidate.category &&
            haversineKm(existing.point, candidate.point) * 1000 < 60,
        );
  ...
  if (services) {
    EMERGENCY_SERVICES_CACHE.set(cacheKey, { savedAt: Date.now(), services });
...
function demoBmdAlert(geometry: unknown): BmdAlert | null {
  ...
  return {
    provider: "bmd",
    id: "demo-weather-warning",
    event: "Rain",
    severity: "Severe",
    headline: "Heavy rainfall warning along your route corridor",
    instruction: "Carry rain protection and avoid low-lying stretches.",
    area: "Route corridor",
