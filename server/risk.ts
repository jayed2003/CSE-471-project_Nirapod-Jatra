...
export function floodStatusFor(point: [number, number], stations: FloodWarning[]): "None" | "Watch" | "Warning" {
  let nearest: FloodWarning | null = null;
  let nearestDistance = Infinity;
  for (const station of stations) {
    const distance = haversineKm(point, station.point);
    if (distance < nearestDistance) { nearestDistance = distance; nearest = station; }
  }
  if (!nearest || nearestDistance > FLOOD_RADIUS_KM || nearest.status === "None") return "None";
  return nearest.status;
}
export function floodStatusForRoute(points: Array<[number, number]>, stations: FloodWarning[]): "None" | "Watch" | "Warning" {
...
export async function fetchLiveRisk(destinationPoint: [number, number], route?: { geometry?: unknown }): Promise<RiskInputs> {
  try {
    ...
    return { aqi, floodStatus, dengueStatus: "None", weatherAlert, weatherDescription: WEATHER_DESCRIPTIONS[current.weathercode] ?? "Unknown conditions", temperature: current.temperature_2m };
  } catch {
    return baselineRisk();
  }
}
