...
export async function fetchJson<T>(url: string, attempts = 2): Promise<T> {
  ...
  throw new Error(
    `Data provider unavailable: ${lastError instanceof Error ? lastError.message : "network error"}`,
  );
}
...
export function normalizeCurrentWeather(current: OpenMeteoResponse["current"]): CurrentWeather {
  return {
...
export function getWeatherForecast(latitude: number, longitude: number) {
  return cached(
    ...
    latitude,
    longitude,
    async (): Promise<WeatherForecastDay[]> => {
      ...
      return data.daily.time
        ...
        .map((date, index) => ({
          ...
          description: weatherDescription(data.daily.weathercode[index]),
        }));
    },
  );
}
...
export function getHourlyWeatherForecast(latitude: number, longitude: number) {
  return cached("hourly-weather", latitude, longitude, async (): Promise<HourlyWeatherPoint[]> => {
    ...
    return data.hourly.time.map((time, index) => ({
      ...
      description: weatherDescription(data.hourly.weathercode[index]),
...
export function normalizeAirQuality(item: AirResponse["list"][number]): AirQuality {
  return {
    aqi: item.main.aqi,
    label: aqiLabel(item.main.aqi),
    ...
    observedAt: new Date(item.dt * 1000).toISOString(),
  };
}
export function getAirQuality(latitude: number, longitude: number) {
  return cached("air-quality", latitude, longitude, async () => {
    const data = await fetchJson<AirResponse>(
      `https://api.openweathermap.org/data/2.5/air_pollution?lat=${latitude}&lon=${longitude}&appid=${apiKey()}`,
    );
    return normalizeAirQuality(data.list[0]);
  });
}
export function getAirQualityForecast(latitude: number, longitude: number) {
  return cached(
    "air-forecast",
    latitude,
    longitude,
    async (): Promise<AirQualityForecastPoint[]> => {
      const data = await fetchJson<AirResponse>(
