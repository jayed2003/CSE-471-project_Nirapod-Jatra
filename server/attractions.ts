...
const GEOGRAPHIC_TYPE_EXCLUDE_PATTERN =
  /\b(cities|city|towns?|villages?|upazil(a|as)|than(a|as)|unions?|municipalit(y|ies)|districts?|divisions?|subdivisions?|arrondissement|boroughs?|wards?|neighbou?rhoods?|localit(y|ies)|suburbs?|metropolis|hamlets?|parish|counties|county|provinces?|states?|countr(y|ies)|areas?)\b/i;

const CATEGORY_KEYWORDS: Array<{ pattern: RegExp; category: AttractionCategory }> = [
  { pattern: /\b(museum|gallery)\b/i, category: "museum" },
  {
    pattern:
      /\b(zoo|aquarium|theme park|amusement park|water park|planetarium|theatres?|theaters?|cinemas?|stadiums?|arenas?)\b/i,
    category: "entertainment",
  },
  {
    pattern:
      /\b(beach|waterfalls?|lakes?|islands?|hills?|mountains?|peaks?|viewpoints?|forests?|wildlife sanctuary|nature reserve|caves?|valleys?|rivers?)\b/i,
    category: "nature",
  },
  { pattern: /\b(parks?|gardens?|botanical)\b/i, category: "park" },
  {
    pattern:
      /\b(temples?|mosques?|churches?|shrines?|mazar|monaster(y|ies)|cathedrals?|synagogues?|pagodas?|stupas?|forts?|fortress|palaces?|castles?|citadel|archaeological site|ruins|monuments?|memorials?|mausoleum|tombs?|heritage site|historic (house|site)|towers?|minar|gates?|walls?|fountains?|squares?|plaz(a|as)|statues?|sculptures?|bridges?|lighthouse|observator(y|ies)|markets?|librar(y|ies)|shrines?)\b/i,
    category: "landmark",
  },
];
...
function classifyAttraction(
  description: string,
): { category: AttractionCategory; kind: string } | null {
  if (
    !description ||
    STRONG_EXCLUDE_PATTERN.test(description) ||
    EXCLUDE_DATE_RANGE_PATTERN.test(description)
  )
    return null;
...
async function queryWikiPage(
  ...
  try {
    const response = await fetch(url, {
      headers: { "user-agent": "WaymarkSafety/1.0 (local development)" },
...
export async function findTouristAttractions(
  point: [number, number],
  radiusMeters = WIKI_MAX_RADIUS_METERS,
): Promise<NearbyAttractionsResult> {
  ...
  if (cached && Date.now() - cached.savedAt < ATTRACTIONS_CACHE_MS)
    return { attractions: cached.attractions, degraded: false };
  ...
  if (radius >= 5_000 && byId.size < MIN_DESIRED_RESULTS) {
    const satellites = await Promise.all(
      RING_BEARINGS.map((bearing) =>
        queryWikiPage(offsetPoint(point, RING_OFFSET_KM, bearing), radius),
      ),
    );
