...
const briefSchema = z.object({ destination: z.string().min(2).max(200), distanceKm: z.number().nonnegative().optional(), durationMin: z.number().nonnegative().optional(), temperature: z.number().optional(), weather: z.string().max(80).optional(), aqi: z.number().int().min(1).max(5).optional(), aqiLabel: z.string().max(30).optional() });
type BriefContext = z.infer<typeof briefSchema>;
...
export async function POST(request: NextRequest) {
  ...
  const context = briefSchema.parse(await request.json());
  if (!key) return NextResponse.json({ brief: fallbackBrief(context), source: "fallback" });
  try {
    ...
    return NextResponse.json({ brief: text, source: "gemini" });
  } catch { return NextResponse.json({ brief: fallbackBrief(context), source: "fallback" }); }
}