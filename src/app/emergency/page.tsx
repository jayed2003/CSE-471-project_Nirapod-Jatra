...
import {
  ...
  type SelectedEmergencyService,
} from "@/components/EmergencyServicesPanel";
...
type Profile = {
  user: { displayName: string; plan?: Plan; safeWord?: SafeWordSetting };
  contacts: ScriptContact[];
  trips: Array<{ _id: string; travelDates?: { end?: string } }>;
};
...
export default function EmergencyPage() {
  ...
  const activeTrip = profile?.trips.find(
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true;
      window.removeEventListener("plan:changed", loadProfile);
    };
  }, []);
    ...
    (async () => {
      ...
      try {
        const result = await apiFetch<{
          ...
          method: "POST",
          body: JSON.stringify({
            message: "Missed check-in: the 6-hour check-in timer expired without confirmation.",
            trigger: "missed-checkin",
            ...(location
              ? {
        ...
        const testNote = result.testMode
          ? " (test mode — SMTP isn't configured yet, so nothing reached a real inbox)"
          : "";
        setCheckinStatus(
          `Check-in window expired. Emailed ${result.emailsSent} of ${result.contactsNotified} emergency contact(s).${testNote}`,
        );
      } catch {
        setCheckinStatus(
          "Check-in window expired, but we couldn't reach the server to notify contacts.",
        );
      }
    })();
  }, [seconds]);
    ...
    try {
      ...
      setStatus(
        result.contactsNotified === 0
          ...
          : `SOS recorded. Emailed ${result.emailsSent} of ${result.contactsNotified} emergency contact(s).${testNote}`,
      );
    } catch {
  ...
  async function startSharing() {
    ...
    const location = await currentLocation();
    if (!location) {
      setShareStatus("Location unavailable. Allow location access to share your live location.");
      return;
    }
    try {
      const result = await apiFetch<{
        ...
        body: JSON.stringify({
          ...
          durationMinutes: CHECK_IN_SECONDS / 60,
        }),
      });
      ...
      setShareStatus(
        result.contactsNotified === 0
          ? "Sharing started. Add an emergency contact so someone gets the link next time."
          : `Sharing started. Emailed the live link to ${result.emailsSent} of ${result.contactsNotified} emergency contact(s).${testNote}`,
      );
      if (navigator.geolocation) {
        shareWatchId.current = navigator.geolocation.watchPosition(
          (position) => {
            ...
            if (!token || now - lastShareUpdateAt.current < SHARE_LOCATION_UPDATE_MIN_INTERVAL_MS)
              return;
      ...
      setShareStatus(
        reason instanceof Error ? reason.message : "Could not start live location sharing.",
      );
    }
  }
  ...
  async function stopSharing() {
    ...
    try {
      await apiFetch("/api/location-share/stop", { method: "POST" });
    } catch {
      ...
      <main className="subpage emergency-page">
        ...
        <section className="emergency-grid">
          <article>
            ...
            <h2>Send SOS</h2>
            <p>{status}</p>
            <button className="danger-button" onClick={() => void sos()}>
              Send emergency SOS
            </button>
          </article>
          <article>
            ...
            <div className="countdown">
              {seconds === null ? "Not scheduled" : formatCountdown(seconds)}
            </div>
            <button onClick={startCheckin}>Start 6-hour check-in</button>
            {seconds !== null && (
              <button className="text-button" onClick={cancelCheckin}>
                Cancel countdown
              </button>
            )}
            ...
            <article>
              ...
              {!sharing ? (
                ...
                <>
                  ...
                  {shareExpiresAt && (
                    <p className="services-status">Live until {shareExpiresAt.toLocaleString()}</p>
                  )}
                  <button className="text-button" onClick={() => void stopSharing()}>
                    Stop sharing
                  </button>
                </>
              )}
          ...
          <article className="emergency-map-card" ref={mapCardRef}>
            ...
            <p>
              {selectedService ? (
                <>
                  Showing <strong>{selectedService.name}</strong> relative to your current location.
                </>
              ) : (
                "Your current location, shown for reference only. Nothing here is shared unless you send an SOS."
              )}
            </p>
            ...
            {!locating && coords && (
              <MapPreview
                ...
                zoom={15}
                secondaryMarker={
                  selectedService ? { point: coords, label: "You are here" } : undefined
                }
              />
            )}
            {!locating && coords && accuracyM !== undefined && (
              <p className="services-status">
                ...
                {accuracyM > 300 &&
                  " — this is a Wi-Fi/IP position estimate, not GPS, so it can land a neighborhood off (even when the browser reports a small accuracy number, the underlying Wi-Fi location database can be stale for this area). Open this page on a phone with GPS enabled for a precise fix, or check Windows Settings → Privacy & security → Location is on for your browser."}
              </p>
            )}
            {!locating && !coords && (
              <p>Location unavailable. Allow location access in your browser to see the map.</p>
            )}
            <button className="text-button" onClick={() => void refreshLocation()}>
              Refresh location
            </button>
            {selectedService && (
              <button className="text-button" onClick={clearSelectedService}>
          ...
          <PremiumGate plan={profile?.user.plan} feature="SOS script generator">
            ...
            {!locating && !coords && (
              <article className="sos-script-card">
                ...
                <h2>SOS script</h2>
                <p>
                  Allow location access to auto-build a script with your exact coordinates and
                  nearest landmark.
                </p>
                <button className="text-button" onClick={() => void refreshLocation()}>
                  Try location again
