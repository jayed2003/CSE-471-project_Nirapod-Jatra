...
export function AirQualityCard({ lat, lon }: { lat: number; lon: number }) {
  ...
  return (
    <article className="env-card air-card" aria-label="Current air quality">
      <header>
        <Wind size={18} />
        <h3>Air quality</h3>
      </header>
      {loading && <p className="env-loading">Loading air quality...</p>}
      {error && <p className="env-error">{error}</p>}
      {data && (
        <>
          <div className={`aqi-readout ${tone}`}>
          ...
          <dl>
            <div>
              <dt>PM2.5</dt>
              <dd>{data.current.pm25} µg/m³</dd>
            </div>
            <div>
              <dt>PM10</dt>
              <dd>{data.current.pm10} µg/m³</dd>
            </div>
            <div>
              <dt>O₃</dt>
              <dd>{data.current.o3} µg/m³</dd>
            </div>
          </dl>
        </>
      )}
