...
export function VoiceSosArm({
  ...
  const dispatchVoiceSos = useCallback(async () => {
    ...
    const situation =
      situations.find((option) => option.type === voiceSituation) ??
    ...
    const script = coordinates
      ? await generateSosScript({ coordinates, situation, callerName })
      : null;
  ...
  const { supported, listening, error, lastHeard } = useSafeWordListener({
    enabled: armed && Boolean(savedPhrase),
    phrase: savedPhrase,
    romanized,
    sensitivity,
    onMatch,
  ...
  return (
    <article className="voice-sos-card">
      ...
      <p className="voice-hint">
        Pick something you would never say by accident — and that won&apos;t alarm anyone standing
        next to you.
      </p>
