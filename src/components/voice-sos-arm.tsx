...
function alertUser() {
  ...
  } catch { /* audio blocked before a user gesture — the vibration and overlay still fire */ }
}

export function VoiceSosArm({ center, accuracyM, contacts, callerName, initialSafeWord }: {
  ...
  const [sensitivity, setSensitivity] = useState<SafeWordSensitivity>(initialSafeWord?.sensitivity ?? "normal");
  ...
  const dispatchVoiceSos = useCallback(async () => {
    ...
    try {
      const result = await apiFetch<{ contactsNotified: number; emailsSent: number }>("/api/sos", {
        method: "POST",
        body: JSON.stringify({
          ...(center ? { location: { type: "Point", coordinates: [center[0], center[1]] }, accuracyM } : {}),
          message: "Voice safe-word SOS",
          situationType: voiceSituation,
          trigger: "voice",
          voice: detection ? { heardText: detection.heard, confidence: detection.confidence } : undefined,
          ...(script ? { script: { speech: script.speech, sms: script.sms, plain: script.plain, degraded: script.degraded } } : {}),
        }),
      });
      setFireStatus(result.contactsNotified === 0 ? "SOS recorded. No emergency contact is saved — call 999 with the script below." : `SOS sent. Emailed ${result.emailsSent} of ${result.contactsNotified} contact(s).`);
    } catch {
      setFireStatus("Offline — SOS queued and will send when the connection returns. Call 999 now using the script below.");
    }
  }, [accuracyM, callerName, center, situations, voiceSituation]);

  const onMatch = useCallback((heard: string, confidence: number) => {
    detectionRef.current = { heard, confidence };
    alertUser();
    if (testMode) { setTestHit(heard); window.setTimeout(() => setTestHit(null), 6000); return; }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (countdown <= 1) { setCountdown(null); void dispatchVoiceSos(); return; }
  ...
  async function saveSafeWord() {
    ...
    setSaveError("");
    try {
      await apiFetch("/api/me/safe-word", { method: "PUT", body: JSON.stringify({ enabled: true, phrase, romanized: romanized.trim() || undefined, sensitivity }) });
      setSavedPhrase(phrase.trim());
    } catch (saveFailure) {
      setSaveError(saveFailure instanceof Error ? saveFailure.message : "Could not save your safe-word.");
    }
    setSaving(false);
  }
  ...
  return (
    <article className="voice-sos-card">
      ...
      <p>Say a Bangla phrase out loud and the app sends your SOS — for moments when you can&apos;t type or unlock the screen.</p>

      {!supported && (
        <p className="voice-sos-warning">
          This browser can&apos;t listen for a safe-word. Use Chrome on Android or desktop. The SOS button and script still work everywhere.
        </p>
      )}
      ...
      <label className="voice-field">
        ...
        <input value={romanized} onChange={(event) => setRomanized(event.target.value)} maxLength={60} placeholder="nil akash" />
        <small>Bangla recognition sometimes returns English letters. Adding the spelling catches those too.</small>
      </label>

      <label className="voice-field">
        <span>Sensitivity</span>
        <select value={sensitivity} onChange={(event) => setSensitivity(event.target.value as SafeWordSensitivity)}>
      ...
      <label className="voice-field">
        ...
        <select value={voiceSituation} onChange={(event) => setVoiceSituation(event.target.value as SituationType)}>
          {situations.map((option) => <option key={option.type} value={option.type}>{option.en}</option>)}
        </select>
      </label>
      ...
      {saveError && <p className="voice-sos-warning" role="alert">{saveError}</p>}

      {savedPhrase && supported && (
        <div className="voice-arm-row">
          <button type="button" className={armed ? "voice-arm armed" : "voice-arm"} onClick={() => { setArmed((value) => !value); setTestHit(null); }}>
            {armed ? <MicOff size={15} /> : <Mic size={15} />} {armed ? "Stop listening" : "Arm voice SOS"}
          </button>
      ...
      {armed && (
        <p className="voice-hint">
          Your microphone is on and speech is sent to your browser&apos;s recognition service for transcription. Listening stops when you leave this page or lock the screen.
        </p>
      )}
      {error && <p className="voice-sos-warning" role="alert">{error}</p>}
      {testHit && <p className="voice-detected"><CheckCircle2 size={15} /> Safe-word detected. Nothing was sent (test mode).</p>}

      {countdown !== null && (
        <div className="sos-countdown-overlay" role="alertdialog" aria-live="assertive" aria-label="Voice SOS triggered">
          <div className="sos-countdown">
            ...
            <button type="button" className="confirm-no" onClick={() => { setCountdown(null); detectionRef.current = null; }}>
      ...
      {(firedScript || fireStatus) && (
        <div className="voice-fired">
          ...
          {firedScript
            ...
            : <p>Your location wasn&apos;t available, so no script could be built. Call 999 and describe where you are.</p>}
          <button type="button" className="text-button" onClick={() => { setFiredScript(null); setFireStatus(""); }}>Dismiss</button>
        </div>
      )}
    </article>
  );
