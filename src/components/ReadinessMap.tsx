...
export function ReadinessMap({ route, polygons, shelter }: ReadinessMapProps) {
  ...
  useEffect(() => {
    ...
    if (warningPolygons.length) {
      const features = warningPolygons.map((polygon) => ({
      ...
      map.addLayer({
        ...
        paint: { "fill-color": "#cf4a4a", "fill-opacity": 0.25 },
      });
      map.addLayer({
        id: "warning-outline",
        type: "line",
        source: "warning-source",
        paint: { "line-color": "#cf4a4a", "line-width": 2 },
      });
    }
    if (shelter) {
      shelterMarker.current?.remove();
      shelterMarker.current = new maplibregl.Marker({ color: "#16a34a" })
        .setLngLat(shelter.point)
        .setPopup(new maplibregl.Popup({ offset: 18 }).setText(shelter.name))
        .addTo(map);
  ...
  return (
    <div
      className="readiness-map"
      ref={element}
      aria-label="Route readiness map with warned areas and nearest shelter"
    />
  );
}
