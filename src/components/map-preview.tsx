...
export function MapPreview({
  ...
  useEffect(() => {
    ...
    markerRef.current = new maplibregl.Marker({ color: "#d95c4a" })
      .setLngLat(center)
    ...
    map.on("error", () =>
      setMessage("Map tiles could not load. Saved map data remains available offline."),
    );
    return () => {
  ...
  useEffect(() => {
    ...
    if (secondaryMarker) {
      if (!secondaryMarkerRef.current)
        secondaryMarkerRef.current = new maplibregl.Marker({ color: "#3fa88c" })
          .setLngLat(secondaryMarker.point)
          .addTo(mapRef.current);
      secondaryMarkerRef.current
        .setLngLat(secondaryMarker.point)
        .setPopup(new maplibregl.Popup({ offset: 24 }).setText(secondaryMarker.label));
    ...
    label,
    zoom,
    secondaryMarker?.point[0],
    secondaryMarker?.point[1],
    secondaryMarker?.label,
  ]);

  return (
    <div className="map-wrap">
      <div className="map-canvas" ref={mapElement} aria-label={`Safety map centered on ${label}`} />
      <span className="map-status">{message}</span>
    </div>
  );
}
