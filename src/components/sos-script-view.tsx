...
export function SosScriptView({
  ...
  async function copyScript() {
    try {
      await navigator.clipboard.writeText(script.plain);
      setCopied(true);
      window.setTimeout(() => setCopied(false), 2000);
    } catch {
      /* clipboard blocked — the text is on screen to read */
    }
  }
    ...
    <div className="sos-script">
      {script.degraded && (
        <p className="sos-script-degraded">
          Landmark and address lookup was unavailable, so this script has coordinates only. The GPS
          numbers below are still exact — read them out.
        </p>
      )}
      ...
      <dl className="sos-script-facts">
        <div>
          <dt>GPS</dt>
          <dd>
            {facts.coordinates.lat.toFixed(5)}, {facts.coordinates.lon.toFixed(5)}
            {facts.coordinates.accuracyM ? ` (±${Math.round(facts.coordinates.accuracyM)} m)` : ""}
          </dd>
        </div>
        <div>
          <dt>Landmark</dt>
          <dd>
            {facts.landmark
              ? `${facts.landmark.name} — ${facts.landmark.distanceM} m ${facts.landmark.bearingEn} of it`
              : "Not confirmed"}
          </dd>
        </div>
        {facts.alternatives.length > 0 && (
          ...
          <div>
            ...
            <dd>
              {facts.alternatives
                .map((alternative) => `${alternative.name} (${alternative.distanceM} m)`)
                .join(" · ")}
            </dd>
          </div>
        )}
        <div>
          <dt>Situation</dt>
          <dd>
            {facts.situation.en} · {facts.situation.bn}
          </dd>
        </div>
        {facts.nearestHospital && (
          <div>
            ...
            <dd>
              {facts.nearestHospital.name} ({facts.nearestHospital.distanceKm} km)
            </dd>
          </div>
        )}
      </dl>
      ...
      <div className="sos-script-actions">
        <button
          ...
          onClick={() => setCallTarget({ label: "National Emergency Hotline", number: "999" })}
        >
      ...
      <div className="sos-script-sms">
        ...
        {contacts.length === 0 && (
          <p className="sos-script-empty">
            No emergency contact saved yet. Add one in your account to enable one-tap SMS.
          </p>
        )}
        {contacts.map((contact) => (
          <a
            key={contact._id ?? contact.phone}
            className="sos-sms-link"
            href={smsHref(contact.phone, script.sms)}
          >
            <MessageSquare size={14} /> <strong>{contact.name}</strong> <span>{contact.phone}</span>
          </a>
        ))}
        {contacts.length > 0 && (
          <small>Opens your phone&apos;s messaging app with the text ready — you tap send.</small>
        )}
      </div>
      ...
      {callTarget && (
        <div className="confirm-overlay" onClick={() => setCallTarget(null)}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="sos-call-title"
            onClick={(event) => event.stopPropagation()}
          >
            <h3 id="sos-call-title">Call {callTarget.label}?</h3>
            <p>
              You&apos;re about to dial <strong>{callTarget.number}</strong>. Keep this screen open
              — the script stays here while you talk.
            </p>
            <div className="confirm-actions">
              ...
              <button
                ...
                onClick={() => {
                  window.location.href = `tel:${callTarget.number}`;
                  setCallTarget(null);
                }}
              >
                <Phone size={14} /> Call
