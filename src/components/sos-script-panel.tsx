...
export function SosScriptPanel({ center, accuracyM, contacts, callerName }: { center: [number, number]; accuracyM?: number; contacts: ScriptContact[]; callerName: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { void primeSosKit(coordinates); }, KIT_REFRESH_MS);
    ...
    try {
      const result = await apiFetch<{ contactsNotified: number; emailsSent: number }>("/api/sos", {
        ...
        body: JSON.stringify({
          ...
          situationType,
          trigger: "button",
          script: { speech: script.speech, sms: script.sms, plain: script.plain, degraded: script.degraded },
        }),
      });
      setDispatchStatus(result.contactsNotified === 0 ? "SOS recorded. Add an emergency contact so someone gets notified next time." : `SOS recorded. Emailed ${result.emailsSent} of ${result.contactsNotified} contact(s). Use the SMS buttons below to text them too.`);
    } catch {
      setDispatchStatus("Offline — the SOS is queued and will send when the connection returns. Call 999 now using the script below.");
    }
  }
  ...
  return (
    <article className="sos-script-card">
      ...
      <div className="situation-picker" role="radiogroup" aria-label="Situation type">
        {situations.map((option) => (
          <button
            ...
            onClick={() => { setSituationType(option.type); setScript(null); }}
          >
      ...
      <label className="sos-note">
        ...
        <input value={note} onChange={(event) => setNote(event.target.value)} maxLength={200} placeholder="e.g. two people injured, one unconscious" />
      </label>

      <button type="button" className="danger-button sos-generate" onClick={() => void generate()} disabled={generating}>
        {generating ? <Loader2 size={16} className="spinning" /> : <FileText size={16} />} {generating ? "Building script..." : "Generate SOS script"}
      </button>
      ...
      {script && (
        <>
          ...
          {dispatchStatus && <p className="sos-dispatch-status" role="status">{dispatchStatus}</p>}
        </>
      )}
    </article>
  );
