...
import { Compass, Landmark, Palette, RefreshCw, Sparkles, Ticket, TreePine, Trees } from "lucide-react";
...
type AttractionsResponse = { attractions: TouristAttraction[]; degraded: boolean; bestTimeToVisit: { text: string; source: "gemini" | "fallback" } };
...
export function TouristExplorer() {
  ...
  async function loadAttractions(targetPlace: Place) {
    const attractionsResponse = await fetch(`/api/attractions?lat=${targetPlace.lat}&lon=${targetPlace.lon}&destination=${encodeURIComponent(targetPlace.displayName)}`);
    if (!attractionsResponse.ok) throw new Error("Could not load attractions for this destination.");
  ...
  async function explore(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!resolvedPlace) {
        ...
        if (geocodeResponse.status === 404) throw new Error("Destination not found. Try a different spelling or a larger nearby city.");
        if (!geocodeResponse.ok) throw new Error("Location service is unavailable. Please try again.");
  ...
  async function retry() {
    ...
    setRefreshing(true);
    try { await loadAttractions(place); } catch { /* keep showing the previous result */ } finally { setRefreshing(false); }
  }
    ...
    <section className="explorer-card">
      <div className="explorer-heading">
        <div>
          ...
          <p>Enter a city or destination to find its popular tourist attractions and the best time to visit.</p>
        </div>
      </div>
      <form onSubmit={(event) => void explore(event)} className="explorer-search">
        <Compass size={18} />
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="City or destination" required />
        <button type="submit" disabled={status === "loading"}>{status === "loading" ? "Exploring..." : "Explore"}</button>
      </form>
      {status === "error" && <p className="services-status">{error}</p>}
      {status === "ready" && place && result && (
        <>
          <p className="services-status">Popular attractions in {place.displayName.split(",")[0]}.</p>
          {result.degraded && (
            <p className="services-status services-degraded">
              Live lookup was slow and results may be incomplete — this is not a confirmed &quot;nothing found&quot;.
              <button type="button" className="text-button" onClick={() => void retry()} disabled={refreshing}><RefreshCw size={13} /> {refreshing ? "Retrying..." : "Retry"}</button>
            </p>
          )}
          ...
          {result.attractions.length === 0 && <p className="services-empty">No well-known attractions found for {place.displayName.split(",")[0]} yet. Try a larger nearby city.</p>}
          {result.attractions.length > 0 && (
            <div className="services-groups">
              {SECTIONS.map((section) => {
                const items = result.attractions.filter((attraction) => section.categories.includes(attraction.category));
