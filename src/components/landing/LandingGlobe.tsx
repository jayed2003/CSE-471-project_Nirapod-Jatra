          ...
          >
            <mesh>
              ...
              <meshStandardMaterial
                color={active ? "#e2a63b" : "#f3eee3"}
                emissive={active ? "#e2a63b" : "#3fa88c"}
            ...
            {active && (
              <mesh>
                ...
                <meshBasicMaterial color="#e2a63b" transparent opacity={0.35} depthWrite={false} />
              </mesh>
            )}
            {showLabel && (
              <Html
              ...
              >
                <span className="globe-label">{destination.name}</span>
              </Html>
            )}
          </group>
        );
...
function FocusRig({ focus }: { focus: GlobeDestination | null }) {
  ...
  const animator = useRef({
    progress: 1,
    from: new THREE.Vector3(0, 0, CAMERA_DISTANCE),
    to: new THREE.Vector3(0, 0, CAMERA_DISTANCE),
  ...
  useEffect(() => {
    ...
    if (!focus) {
      animation.to.copy(basePosition.current);
      animation.toTarget.set(0, 0, 0);
  ...
  if (!supported)
    return (
      <div className="globe-fallback" role="status">
        <p>The interactive 3D globe needs WebGL2, which this browser does not currently support.</p>
        <p>You can still use destination search and live navigation below.</p>
      </div>
    );
  return (
    <div className="globe-canvas" aria-label="Interactive 3D globe of destinations">
      <Canvas
        dpr={[1, 1.5]}
