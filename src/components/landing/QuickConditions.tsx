...
export function QuickConditions({ onDestinationSelected }: Props) {
  ...
  const [message, setMessage] = useState("Search any destination for its live weather and air quality. No account needed, and nothing is saved.");
  ...
  return (
    <section className="quick-conditions" id="quick-search" aria-label="Quick destination conditions">
      <div className="quick-heading"><div><p className="eyebrow">Quick destination check</p><h2>Where are you going?</h2></div><p>{message}</p></div>
      <form onSubmit={search} className="quick-search-form"><div className="search-row"><MapPin size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="City, station, or address" required aria-label="Destination" /><button type="submit">{loading ? "Searching..." : "Check conditions"} <ArrowRight size={16} /></button></div></form>
      {place && <div className="environment-grid landing-environment-grid" aria-label={`Live environmental conditions for ${place.name}`}>
        ...
        <ForecastCard key={`forecast-${place.lat}-${place.lon}`} lat={place.lat} lon={place.lon} />
        <AirQualityForecastCard key={`air-forecast-${place.lat}-${place.lon}`} lat={place.lat} lon={place.lon} />
      </div>}
    </section>
  );
}
