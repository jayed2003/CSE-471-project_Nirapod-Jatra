...
export function SosButton() {
  ...
  async function sendSos() {
    try {
      ...
      const response = await fetch(`${apiBase}/api/sos`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ location: position ? { type: "Point", coordinates: [position.coords.longitude, position.coords.latitude] } : undefined, message: "Emergency SOS requested" }) });
