...
export function SosButton() {
  ...
  async function sendSos() {
    if (!getToken()) {
      setMessage("Sign in to send an SOS");
      return;
    }
    ...
    try {
      ...
      const testNote = result?.testMode
        ? " (test mode — real email delivery is not configured yet)"
        : "";
      setMessage(
        result?.queued
          ? "SOS queued for sync"
          : `SOS sent${typeof result?.emailsSent === "number" ? ` to ${result.emailsSent} contact(s)` : ""}${testNote}`,
      );
    } catch (failure) {
      setMessage(
        failure instanceof Error && /Authentication/i.test(failure.message)
          ? "Sign in to send an SOS"
          : "SOS queued when connection returns",
      );
    }
  }
