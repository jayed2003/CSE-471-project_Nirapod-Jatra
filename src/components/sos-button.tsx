...
export function SosButton() {
  ...
  async function sendSos() {
    ...
    try {
      ...
      const result = await apiFetch<{ queued?: boolean; contactsNotified?: number; emailsSent?: number }>("/api/sos", {
        method: "POST",
        body: JSON.stringify({
      ...
      setMessage(result?.queued ? "SOS queued for sync" : `SOS sent${typeof result?.emailsSent === "number" ? ` to ${result.emailsSent} contact(s)` : ""}`);
    } catch (failure) {
