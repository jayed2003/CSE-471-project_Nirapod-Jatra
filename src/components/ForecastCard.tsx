...
export function ForecastCard({ lat, lon }: { lat: number; lon: number }) {
  ...
  return (
    <article className="env-card forecast-card" aria-label="Five day weather forecast">
      <header>
        <CalendarDays size={18} />
        <h3>5-day outlook</h3>
      </header>
      {loading && <p className="env-loading">Loading forecast...</p>}
      {error && <p className="env-error">Weather data unavailable.</p>}
      {data && (
        <ul>
