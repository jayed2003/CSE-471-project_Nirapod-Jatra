...
type SosKit = {
  point: [number, number];
  callerName: string;
  address: SosScript["facts"]["address"];
  landmark: Landmark | null;
  savedAt: number;
};
...
const speakCoordinateBn = (value: number) =>
  value
    .toFixed(4)
    .split("")
    .map((character) =>
...
export function composeOfflineScript(input: {
  ...
  const usable =
    input.kit && haversineMeters(input.kit.point, kitPoint) <= KIT_REUSE_METERS ? input.kit : null;
  const landmark = usable?.landmark ?? null;
  const place = usable?.address
    ? [usable.address.road, usable.address.area, usable.address.city].filter(Boolean).join(", ")
    : null;
  ...
  const speech = [
    ...
    landmark
      ? `কাছের পরিচিত জায়গা ${landmark.name} — সেখান থেকে প্রায় ${toBanglaDigits(landmark.distanceM)} মিটার ${landmark.bearingBn}।`
      : null,
  ...
  const plain = [
    ...
    landmark
      ? `Nearest landmark (cached): ${landmark.name}, about ${landmark.distanceM} m ${landmark.bearingEn} of it`
  ...
  return {
    speech,
    sms,
    smsLatin: `SOS: ${situation.en}. ${callerName}. ${mapsUrl}`,
    plain,
    degraded: true,
 ...
 */
export async function generateSosScript(input: {
  coordinates: { lat: number; lon: number; accuracyM?: number };
  situation: SituationOption;
  note?: string;
  callerName: string;
  ...
  try {
    ...
    await cacheValue<SosKit>(KIT_KEY, {
      point: [coordinates.lon, coordinates.lat],
      callerName: script.facts.callerName,
      address: script.facts.address,
      landmark: script.facts.landmark,
      savedAt: Date.now(),
    });
    return script;
  } catch {
...
export async function primeSosKit(coordinates: { lat: number; lon: number }) {
  try {
    const script = await apiFetch<SosScript>("/api/sos/script", {
      method: "POST",
      body: JSON.stringify({ location: coordinates, situationType: "unknown", language: "bn" }),
    });
    await cacheValue<SosKit>(KIT_KEY, {
      point: [coordinates.lon, coordinates.lat],
      callerName: script.facts.callerName,
